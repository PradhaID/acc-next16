...
export default function EditGroupPage() {
  ...
  useEffect(() => {
    document.title = `Edit Group - ${process.env.NEXT_PUBLIC_APP_NAME || "Boilerplate"}`;
  }, []);
    ...
    <div className="max-w-full mx-auto space-y-6 pb-10">
      <PageHeader
        ...
        subtitle={
          group ? (
            <>
              Editing: <span className="font-bold text-emerald-600">{group.name}</span>
            </>
          ) : undefined
        }
        actions={
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-stone-900/80 p-4 rounded-2xl border border-gray-200 dark:border-stone-700/50 shadow-sm">
            ...
            <div className="space-y-3 text-[11px]">
              <div className="flex justify-between py-1.5 px-2 rounded-lg bg-gray-50 dark:bg-stone-800/40">
                <span className="text-gray-400 font-bold">ID</span>
                <span className="font-mono font-bold text-gray-700 dark:text-stone-300">{group?._id.slice(-8)}</span>
              </div>
              <div className="flex justify-between py-1.5 px-2 rounded-lg bg-gray-50 dark:bg-stone-800/40">
              ...
              {group?.description && (
                <div className="flex justify-between py-1.5 px-2 rounded-lg bg-gray-50 dark:bg-stone-800/40">
                  ...
                  <span className="font-bold text-gray-700 dark:text-stone-300 truncate ml-2 max-w-[120px]">{group.description}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-stone-900/80 p-4 rounded-2xl border border-gray-200 dark:border-stone-700/50 shadow-sm text-xs">
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-stone-700/50">
              <Link
                href={`/system/group/role/${id}`}
                className="flex items-center justify-center gap-1.5 py-2 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
              >
