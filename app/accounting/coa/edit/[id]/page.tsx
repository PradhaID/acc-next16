...
function flattenCoa(nodes: ParentOption[], parentId: string | null = null, excludeId?: string): ParentOption[] {
  return nodes
    .filter((n) => {
      if (excludeId && n._id === excludeId) return false;
      return (n.parent || null) === parentId;
    })
    .sort((a, b) => Number(a.code) - Number(b.code))
    .reduce((acc: ParentOption[], child) => [...acc, child, ...flattenCoa(nodes, child._id, excludeId)], []);
}
...
export default function EditCOAPage() {
  ...
  useEffect(() => { document.title = "Edit COA - AccNext"; }, []);
  useEffect(() => {
    Promise.all([
      fetch("/api/accounting/coa?all=true").then((r) => r.json()),
      fetch(`/api/accounting/coa?id=${id}`).then((r) => r.json()),
    ])
      .then(([parentsData, coaData]) => {
        setParents(parentsData);
        setCoa(coaData);
        const parentId = coaData.parent
          ? typeof coaData.parent === "object" && coaData.parent !== null
            ? (coaData.parent as { _id: string })._id
            : coaData.parent
          : "";
        setParent(parentId);
        setCode(Number(coaData.code));
        setName(coaData.name);
        setDescription(coaData.description || "");
        setCategory(coaData.category);
        setPosition(coaData.position || detectCategory(String(coaData.code || "")) ? "Db" : "Cr");
        setIsActive(coaData.isActive !== false);
      })
      .catch(() => {
  ...
  const handleCodeChange = (value: number) => {
    ...
    if (detected) {
      ...
      const map: Record<string, "Db" | "Cr"> = {
        Asset: "Db", Expense: "Db",
        Liability: "Cr", Equity: "Cr", Revenue: "Cr",
      };
      setPosition(map[detected] || "Db");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErrors(err.errors || { form: "Failed to update account" });
        return;
      }
  ...
  const isDuplicateCode = parents.some(
    (p) => Number(p.code) === code && p._id !== id
  );
    ...
    <div className="max-w-full mx-auto space-y-6 pb-10">
      <PageHeader
        ...
        subtitle={
          coa ? (
            <>
              Editing: <span className="font-bold text-emerald-600">{coa.code} – {coa.name}</span>
            </>
          ) : undefined
        }
        actions={
          <div className="flex items-center gap-2">
            ...
            <button
              type="submit"
              form="coa-edit-form"
              disabled={submitLoading || isDuplicateCode}
      ...
      <form id="coa-edit-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Left column — form */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  {flattenCoa(parents, null, id).map((p) => {
              ...
              <div className="space-y-1.5">
                ...
                />
                {isDuplicateCode && (
                  <p className="text-[10px] text-red-600 font-bold ml-1">Code already exists</p>
                )}
                {errors.code && (
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-700/50 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                If disabled, this account won&apos;t be available for transactions.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-200 dark:border-gray-700/50 shadow-sm text-xs">
            ...
            <p className="text-gray-500 leading-relaxed italic mb-3">
              Changes to category and position will affect all child accounts if this is a parent account.
            </p>
            <div className="pt-3 border-t border-gray-100 dark:border-gray-800 space-y-2">
              <div className="flex justify-between text-[10px]">
                <span className="text-gray-400">Account ID:</span>
                <span className="font-mono font-bold text-gray-700 dark:text-gray-300">{coa?._id.slice(-8)}</span>
              </div>
              <div className="flex justify-between text-[10px]">
                <span className="text-gray-400">Code:</span>
                <span className="font-mono font-bold text-gray-700 dark:text-gray-300">{coa?.code}</span>
              </div>
            </div>
          </div>
        </div>
