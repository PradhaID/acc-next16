                    ...
                    return (
                      <div key={index} className="group relative rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 aspect-square">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                          <button
                            onClick={(e) => { e.stopPropagation(); setViewerUrl(item.url); setShowViewerModal(true); }}
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); setEvidenceToDelete(item.url); setShowDeleteEvidenceModal(true); }}
      ...
      {showDeleteEvidenceModal && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center">
            ...
            <h3 className="text-lg font-bold mb-2 text-gray-900 dark:text-white">Remove Evidence?</h3>
            <p className="text-gray-500 text-sm mb-8">Are you sure you want to permanently delete this evidence document? This action cannot be undone.</p>
            <div className="flex gap-3">
              <button
                onClick={() => handleDeleteEvidence(evidenceToDelete)}
                disabled={actionLoading}
                className="flex-1 bg-red-600 text-white py-3 rounded-2xl font-bold hover:bg-red-700 transition-all active:scale-95 disabled:opacity-50"
              >
                {actionLoading ? "Removing..." : "Remove Now"}
              </button>
              <button
                onClick={() => { setShowDeleteEvidenceModal(false); setEvidenceToDelete(""); }}
