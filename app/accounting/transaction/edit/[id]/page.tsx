...
export default function EditTransactionPage({
  ...
  useEffect(() => { document.title = "Edit Transaction - AccNext"; }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([txnRes, accRes]) => {
        ...
        setAccounts(accs);
        setType(txn.type);
        setEffectiveDate(txn.effectiveDate ? txn.effectiveDate.split("T")[0] : "");
  ...
  const addLine = useCallback(() => {
    setLines((prev) => [
      ...prev,
      { key: crypto.randomUUID(), account: "", accountLabel: "", position: "Db", amount: 0 },
    ]);
  }, []);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (res.ok) {
          router.push(`/accounting/transaction/detail/${id}?updated=true`);
        } else {
          const data = await res.json();
          setErrors({ form: data.message || "Failed to update transaction" });
        }
      } catch {
    ...
    [id, type, effectiveDate, reference, information, lines, validate, router]
  );
  ...
  if (transaction.status !== "Pending") {
    return (
      <div className="max-w-full mx-auto space-y-6 pb-10">
        ...
        />
        <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-700/50 shadow-sm text-center py-16">
          <p className="text-gray-500 font-medium">
            This transaction cannot be edited because it is <strong>{transaction.status}</strong>.
          </p>
          <Link
            href={`/accounting/transaction/detail/${id}`}
            className="mt-4 inline-flex items-center gap-2 text-emerald-600 font-bold hover:underline"
          >
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 pb-10">
      ...
      <form id="txn-edit-form" onSubmit={handleSubmit} className="space-y-4">
        {/* Header card */}
        <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-200 dark:border-gray-700/50 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Left column: code, type/date, ref, memo */}
            <div className="flex flex-col space-y-3">
              ...
              <div className="space-y-1">
                ...
                <div className="w-full px-3 py-2 bg-gray-100 dark:bg-gray-800/50 rounded-xl text-sm font-bold text-gray-700 dark:text-gray-300">
                  {transaction.code}
                </div>
              </div>
