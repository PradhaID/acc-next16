...
export default function CategoryAdd() {
    ...
    useEffect(() => {
        fetch("/api/content/category?all=true&active=true")
            .then(res => res.json())
            .then(res => {
                if (res.success) {
                ...
                actions={
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        >
                            ...
                            {loading ? t("common.loading") : t("content.saveCategory")}
                        </button>
                    </div>
                }
            />

            <form id="category-add-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                {/* Left column — form */}
                        ...
                        <div className="w-full h-40 border-2 border-dashed rounded-xl flex items-center justify-center overflow-hidden bg-gray-50 dark:bg-stone-800/40 border-gray-200 dark:border-stone-700/50">
                            {previewImage ? (
                                ...
                                <div className="text-center p-4">
                                    <span className="text-xs text-gray-400">{t("content.noImage")}</span>
                                </div>
                            )}
                        </div>
                        <input
